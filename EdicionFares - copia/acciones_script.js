...
function iniciarf(){
    ...
    // cpaturar el elemento HTMl con el id porcentajev
...
function calcularPorcentaje(costof, porcentaje){
    ...
    // Retornar el precio de venta a la funcion
    return Pventa;
}
...
function mostrarpventa(){
    ...
    if(vcosto.value != '' && vporcentaje.value != ''){
        // Llamar a la funcion calcularPorcentaje y almacenar el 
...
function iniciarf(){
    //Capturar el elemto HTML con el id costop
     vcosto = document.getElementById('costop');
...
function mostrarImagen(){
   ...
   // Asignar el input de tipo filr a la variable archivof
    ...
    //Ejecutar la funciona anonima cuando el archivo se cargue
    leerarchivo.onload = function(){
        // Asignar la direccion al archivo al atributo src de img
        // para mostrar la imagen cargada
        verimagen.src = leerarchivo.result;
    }
    //Si existen datos en la variable archivof entonces
    if(archivof){
        // Leer el archivo
